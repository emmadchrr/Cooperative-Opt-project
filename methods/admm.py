...
def ADMM(X, Y, X_selected, A, nu, sigma2, W, n_epochs,  beta):
    """
    ADMM Implementation
    """

    num_selected = len(X_selected)
    a = len(A) # Number of selected points
    ...
    for i in range(a):
        for j in neighbors[i]:
            ...
            if i < j:
                s = np.random.rand(num_selected) 
                y_k[i, j] = s
                y_k[j, i] = s  

    iteration = 0
    deviation_alpha_star = []  
    alpha_list = []
    alpha_mean_list = []
...
if __name__ == "__main__":
    ...
    start = time.time()
    alpha_optim, alpha_list, alpha_mean_list, opt_gaps = ADMM(x_n, y_n, x_selected, A, nu, sigma2,W, n_epochs,  beta)
